...
public class ResultMapping {
    ...
    public void setStatementName(String statementName) {
        if (statementName != null && (columnName.indexOf('=') >= 0 || columnName.indexOf(',') >= 0)) {
            String str = columnName.trim();
            ...
            if (tc % 2 != 0) {
                throw new RuntimeException("Wrong prop-col pairs in " + columnName);
            }
            columnMap = new String[tc];
            for (int i = 0; i < tc; i++) {
                columnMap[i] = st.nextToken();
            }
