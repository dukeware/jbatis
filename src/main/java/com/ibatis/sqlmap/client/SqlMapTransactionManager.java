...
public interface SqlMapTransactionManager {
    ...
    /**
     * Ends a transaction and rolls back if necessary. If the transaction has been started, but not committed, it will
     * be rolled back upon calling endTransaction().
     *
     * @throws SQLException
    ...
    /**
     * Returns the current connection in use. If no connection exists null will be returned. There may be no connection
     * if no transaction has been started, and if no user provided connection has been set.
     *
